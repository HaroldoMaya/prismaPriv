function [B SSE FIT]=logisticFit2(x,y,alpha,base,kindFit)
%
...
dim = sum(alpha)+3;            % Vector dimension.
index = find(alpha)+3;
...
for i = 1 : size_pop
    B = base;
    B([1 2 3 index]) = B([1 2 3 index]) + V_pop(i, :);
    y_hat = f(x,B);
    if (B(5) <= 0)
       y_hat = inf;
    end
...
for t = 2 : gen
    ...
    for i = 1 : size_pop
        B = base;
        B([1 2 3 index]) = B([1 2 3 index]) + V_off(i, :);
        y_hat = f(x,B);
        if (B(5) <= 0)
           y_hat = inf;
        end
...
B = base;
B([1 2 3 index]) = B([1 2 3 index]) + vBest;
