function [vBest y_hat err SSE FIT]=logisticfit(x,y,k,kindFit,forceValues,penBelow,penAbove)
%
...
if(forceValues)
    ...
    V_pop(:,2) = V_pop(:,2) + 0.1*((max(y) - min(y))/(max(x) - min(x)));
    V_pop(:,3) = V_pop(:,3) + (max(x) - min(x))/2;
    if k>3
...
for t = 2 : gen
    ...
    elseif k == 5
        st = sprintf('%.4g + (%.4g %c %.4g)/{{1 + (%cv/%.4g)^(%c%.4g)}^%.4g}', B(4), B(1), sinal3, abs(B(4)), sinal1, abs(B(2)), sinal2, abs(B(3)),B(5));
    elseif k == 6
        st = sprintf('%.4g + (%.4g %c %.4g)/{{%.4g + (%cv/%.4g)^(%c%.4g)}^%.4g}', B(4), B(1), sinal3, abs(B(4)), B(6), sinal1, abs(B(2)), sinal2, abs(B(3)),B(5));
    end
