function [bestGlobalPos y_hat err SSE FIT]=logisticfit(x,y,k,kindFit,forceValues,penBelow,penAbove)
%
...
if(forceValues)
    position(:,1) = position(:,1) + base(1);
    position(:,2) = position(:,2) + base(2);
    position(:,3) = position(:,3) + base(3);
    if k>3
        position(:,4) = position(:,4) + base(4);
    end
    if k>4
        position(:,5) = position(:,5) + base(5);
    end
    if k>5
        position(:,6) = position(:,6) + base(6);
    end
end
