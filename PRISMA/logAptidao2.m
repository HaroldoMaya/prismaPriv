function [aptPop, ssePop, BPop] = logAptidao2(pop, y, x, base, kindFit)
%% Função de aptidão
...
%   - flagRobusta: Indicativo se a regressão será robusta (IWLS)
    ...
    for i = 1:nPop % Fazer para cada linha de alpha.
        ...
        %% Resolver B.
        [B SSE FIT] = logisticFitDE2(x,y,pop(i, :),base,kindFit);
...
%             b6 = num2str(1);
%         end
%         fStr = sprintf('@(B,x) %s + (B(1)-%s)./((%s + exp(-B(3)*(x - B(2)))).^%s)',b4,b4,b6,b5);
%         f = str2func(fStr);
% 
%         options = optimoptions('lsqcurvefit','MaxIter', 10000, 'MaxFunEvals', 10000, 'Display','off');
%         pFit = lsqcurvefit(f , base, x, y, [], [], options);
%         B = pFit;
%         
%         y_hat = f(pFit,x);
