function [vBest y_hat err SSE FIT]=logisticfit1DE(x,y,k,kindFit,forceValues,penBelow,penAbove)
%
...
B1 = 0.7;           % Initial mutation factor [0 ~~ oo]
B2 = 0.6;            % Final mutation factor [0 ~~ oo]
...
if(forceValues)
    ...
    V_pop(:,2) = V_pop(:,2) + p(indx,1);
    V_pop(:,3) = V_pop(:,3) + 4/max(y)*((p(indx+1,2) - p(indx-1,2))/(p(indx+1,1) - p(indx-1,1)));
    if k>3
        V_pop(:,4) = V_pop(:,4) + min(y);
    end
    if k>4
        V_pop(:,5) = V_pop(:,5) + 1;
    end
...
elseif k == 4
    f = @(v,B) ...
        B(4) + (B(1)-B(4))./(1 + exp(-B(3)*(v - B(2))));
elseif k == 5
    f = @(v,B) ...
        B(4) + (B(1)-B(4))./((1 + exp(-B(3)*(v - B(2)))).^B(5));
elseif k == 6
    f = @(v,B)...
...
for i = 1 : size_pop
    ...
    [fitPop(i), ssePop(i)] = fitValue(y, y_hat, k, kindFit,penBelow,penAbove);
...
for t = 2 : gen
    ...
    for i = 1 : size_pop
        ...
        [fitOff(i), sseOff(i)] = fitValue(y, y_hat, k, kindFit, penBelow, penAbove);
    ...
    y_hat = f(x,B);
    if k == 3
        st = sprintf('%.4g/{1 + exp[%c%.4g*(x %c %.4g)]}', B(1), sinal1, abs(B(3)), sinal2, abs(B(2)));
    elseif k == 4
        st = sprintf('%.4g + (%.4g %c %.4g)/{1 + exp[%c%.4g*(x %c %.4g)]}', B(4), B(1), sinal3, B(4), sinal1, abs(B(3)), sinal2, abs(B(2)));
    elseif k == 5
        st = sprintf('%.4g + (%.4g %c %.4g)/{{1 + exp[%c%.4g*(x %c %.4g)]}^%.4g}', B(4), B(1), sinal3, abs(B(4)), sinal1, abs(B(3)), sinal2, abs(B(2)),B(5));
    elseif k == 6
...
function [fit, sse] = fitValue(y_data, y_pred, NParam, kindFit, limInf, limSup)
    N = numel(y_data);
    err = y_data - y_pred;
